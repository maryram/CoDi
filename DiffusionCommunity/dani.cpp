...
void DANI::runDANI() {
	...
	for (int i = 0; i < Total.size(); i++) {
		...
		//for (int j = 0; j < Total[i].ranklist.size(); j++) {

		
				for (iter=Total[i].ranklist.begin(); iter!=Total[i].ranklist.end();++iter) {
					   
		//	Rank << endl << "rank[" << i << "][" << iter->first  << "]:\t" << iter->second.rank;
...
weights[count].value = ((pow((iter->second.rank / rankI), rankCoef2))* (pow((Subscription /Union ),2)));
				...
				//Rank << "\t" << Subscription << "\t" << Union<< "\t" << weights[count].value;
				//Hey << weights[count].start<<"\t"<< weights[count].end<<"\t"<<weights[count].value << "\n";
				count++;

		}
		//Rows << "row [" << i << " ] : " << rankI << endl;
...
void DANI::GenerateGroundTruth(const int& TNetwork, const int& NNodes,
	...
	case 0:
		...
		GroundTruth = TKronMtx::GenFastKronecker(SeedMtx, (int) TMath::Log2(
				NNodes), NEdges, true, 0);
	...
	while (C.Len() < 2) {
		...
		while (true) {
			...
			for (int e = 0; e < NI.GetOutDeg(); e++) {
				...
				if (TInt::Rnd.GetUniDev() > beta + std_beta
						* TFlt::Rnd.GetNrmDev())
					continue;
				...
				if (InfectedBy.IsKey(NId) && InfectedBy.GetDat(NId).Val
						== DstNId)
					continue;
				...
				case 1:
					...
					sigmaT = sigmaT = p * TInt::Rnd.GetPowerDev(alpha)
							+ (1 - p) * TInt::Rnd.GetPowerDev(alpha2);
					while (!(sigmaT >= 1 && sigmaT < 100)) {
						sigmaT = p * TInt::Rnd.GetPowerDev(alpha) + (1 - p)
								* TInt::Rnd.GetPowerDev(alpha2);
				...
				if (std_waiting_time > 0)
					sigmaT = TFlt::GetMx(0.0, sigmaT + std_waiting_time
							* TFlt::Rnd.GetNrmDev());
	...
	if (PercRndRemoval > 0) {
		for (int i = KeyV.Len() - 1; i >= 0; i--) {
			if (TFlt::Rnd.GetUniDev() < PercRndRemoval) {
				...
				for (TIntPrIntH::TIter EI = EdgesUsedC.BegI(); EI
						< EdgesUsedC.EndI(); EI++) {
					if ((KeyV[i] == EI.GetKey().Val1 && C.IsNode(
							EI.GetKey().Val2) && C.GetTm(KeyV[i]) < C.GetTm(
							EI.GetKey().Val2)) || (KeyV[i] == EI.GetKey().Val2
							&& C.IsNode(EI.GetKey().Val1) && C.GetTm(KeyV[i])
							> C.GetTm(EI.GetKey().Val1))) {
...
void DANI::SavePajek(const TStr& OutFNm) {
	...
	for (THash<TInt, TNodeInfo>::TIter NI = NodeNmH.BegI(); NI < NodeNmH.EndI(); NI++) {
		...
		fprintf(F, "%d \"%s\" ic Blue x_fact %f y_fact %f\r\n",
				NI.GetKey().Val, I.Name.CStr(), TMath::Mx<double>(log(
						(double) I.Vol) - 5, 1), TMath::Mx<double>(log(
						(double) I.Vol) - 5, 1));
	}
	fprintf(F, "*Arcs\r\n");
...
void DANI::SaveEdgeInfo(const TStr& OutFNm) {
	...
	for (THash<TIntPr, TEdgeInfo>::TIter EI = EdgeInfoH.BegI(); EI
			< EdgeInfoH.EndI(); EI++) {
...
void DANI::SaveObjInfo(const TStr& OutFNm) {
	...
	for (THash<TIntPr, TEdgeInfo>::TIter EI = EdgeInfoH.BegI(); EI
		...
		if (Objective.Len() == 0) {
			...
			Objective.Add(Objective[Objective.Len() - 1]
					+ EI.GetDat().MarginalGain);
			Bound.Add(EI.GetDat().MarginalBound
					+ Objective[Objective.Len() - 1]);
		}
	}
...
void DANI::SaveCascades(const TStr& OutFNm) {
	...
	for (int i = 0; i < CascV.Len(); i++) {
		TCascade &C = CascV[i];
		int j = 0;
		for (THash<TInt, THitInfo>::TIter NI = C.NIdHitH.BegI(); NI
				< C.NIdHitH.EndI(); NI++, j++) {
			if (j > 0)
